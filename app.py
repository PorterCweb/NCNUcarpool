            ...
            if web_passenger_len != 1:
                ...
                for i in range(1,web_passenger_len):
                    ...
                    if passenger_case_date>=now_date or passenger_case_launchdate == now_date:
                        ...
                        if passenger_sheet[i][19] == '':
                            passenger_sheet[i][19] == '無'
                            print(driver_sheet[i])
                        else:
                            pass
                        if int(passenger_sheet[i][13]) <= int(passenger_sheet[i][5]) or int(passenger_sheet[i][13])== 0:
                            web_passenger_data_case={
                                ...
                                "header": {
                                    ...
                                    {
                                        "type": "text",
                                        "text": f"司機：{passenger_sheet[i][19]}",
                                        "color": "#000000",
                                        "size": "xs"
                                    },
                                    {
                    ...
                    with ApiClient(configuration) as api_client:
                        ...
                        if int(target_row[14]) != target_row[5]:
                            ...
                            if driver_user_id != 'Checked':
                                ...
                                driver_sheet_id.update(f'N{i+1}:S{i+1}:T{i+1}', [[int(target_row[14])+1, new_id, new_name]])
                        else:
                            line_bot_api.reply_message(
                                ReplyMessageRequest(
                                    reply_token = event.reply_token,
                    ...
                    if passenger_case_date > now_date or passenger_case_launchdate == now_date:
                        confirm_template = ConfirmTemplate(
                            ...
                            actions=[ #一定只能放兩個Action
                                ...
                                MessageAction(label='我想當司機！', text='我想當司機！', data=f'passenger_bedriver{i}')   
                            ]
                        )
                        template_message = TemplateMessage(
                    ...
                    with ApiClient(configuration) as api_client:
                        ...
                        if target_row[13] != target_row[5]:
                            ...
                            if passenger_user_id != 'Checked':
                                ...
                                passenger_sheet_id.update(f'N{i+1}:O{i+1}:P{i+1}', [[int(target_row[13])+1, new_id, new_name]])
                        else:
                            line_bot_api.reply_message(
                                ReplyMessageRequest(
                                    reply_token = event.reply_token,
            ...
            if event.postback.data == f'passenger_bedriver{i}':
                ...
                with passenger_lock:
                    ...
                    if target_row[19] == '':
                        ...
                        passenger_sheet_id.update(f'T{i+1}:U{i+1}', [[passenger_Sure_name, passenger_user_id]])
                        line_bot_api.reply_message(
                                ReplyMessageRequest(
                                    reply_token = event.reply_token,
                                    messages = [TextMessage(text=f'已幫您預約，記得透過LineID聯繫活動發起人!\n發起人LineID：{passenger_sheet[i][9]}')]
                                )
                            )
                    else:
                        line_bot_api.reply_message(
